...
//        keywords=Util.replaceStrParam(keywords);
        return new ResultObject<>(MessageCode.CODE_SUCCESS,Util.getLoginToken(request).getLanguage(),
                programService.list(keywords,page,size,language));
    }
    ...
    public ResultObject<BProgramDetailVo> detail(
        ...
        return  new ResultObject<>(MessageCode.CODE_SUCCESS,Util.getLoginToken(request).getLanguage(),programService.detail(id, Util.getLoginToken(request).getLanguage()));
    }
    ...
    public ResultObject update(
            HttpServletRequest request,
            @ApiParam("解决方案id") @PathVariable Integer id,
            @RequestBody @Valid BProgramAddQo qo,
    ...
    public void CheckHomeLocation(
            ...
            @ApiParam("语言012 中英日") @RequestParam(required = false) Integer language
    )throws Exception{
//        return new ResultObject<>(MessageCode.CODE_SUCCESS,Util.getLoginToken(request).getLanguage(),
//                programService.checkHomeLocation(homeLocation,programId));
        response.getWriter().println(JSONObject.fromObject(new ResultObject(MessageCode.CODE_SUCCESS,
                Util.getLoginToken(request).getLanguage(), programService.checkHomeLocation(homeLocation,language,programId))).toString());
    }
}
