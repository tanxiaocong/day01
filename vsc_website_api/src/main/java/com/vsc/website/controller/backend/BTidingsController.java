...
public class BTidingsController {
    ...
    public ResultObject<PageBean<BTidingsListVo>> list(
            ...
            @ApiParam(value = "页数",required = true) @RequestParam(required = true) int page,
            @ApiParam(value = "数量",required = true) @RequestParam(required = true) int size,
            @ApiParam("语言 012 中英日") @RequestParam(required = false) Integer language
    ) throws Exception {
//        keywords = Util.replaceStrParam(keywords);
        return new ResultObject<>(MessageCode.CODE_SUCCESS, Util.getLoginToken(request).getLanguage(),
                tidingsService.list(keywords, language, page, size));
    }
